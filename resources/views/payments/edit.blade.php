                ...
                <div class="row g-3">
                        ...
                        <select name="employee_id" id="employee_id" class="form-select" required>
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->id }}"
                                    {{ $employee->id == $payment->employee_id ? 'selected' : '' }}>
                                    {{ $employee->full_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="method" id="method" class="form-select" required>
                            <option value="Efectivo" {{ $payment->method == 'Efectivo' ? 'selected' : '' }}>Efectivo
                            </option>
                            <option value="Transferencia" {{ $payment->method == 'Transferencia' ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <label for="created_by" class="form-label fw-bold text-danger">ID del creador</label>
                        <input type="number" name="created_by" id="created_by" class="form-control"
                            value="{{ old('created_by', $payment->created_by) }}">
                    </div>
